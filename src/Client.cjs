...
class Client {
  ...
  async startTLS(tlsOptions = this.options.tlsOptions) {
    ...
    await wrapNativeCall(() => native.startTLS(this.nativeHandle, normalized));
  }

  async search(baseDN, options = {}, controls) {
    const encodedControls = encodeControls(controls);
    const result = await wrapNativeCall(() => native.search(this.nativeHandle, {
      baseDN,
  ...
  async *searchPaginated(baseDN, options = {}, controls) {
    ...
    do {
      const page = await wrapNativeCall(() => native.search(this.nativeHandle, {
        baseDN,
        options: {
